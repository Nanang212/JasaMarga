const { Employee, Education, EmployeeFamily, EmployeeProfile } = require('../models');
...
class EmployeeController {
  ...
  static createValidation = [
    body('nik').isString().isLength({ max: 50 }).custom(async (item) => {
  ...
  static updateValidation = [
    ...
    body('nik').optional().isString().isLength({ max: 50 }).custom(async (item) => {
      const isExists = await Employee.findOne({ where: { nik: item } });
      if (isExists) {
        throw new Error(messages.GENERAL.UNIQUE);
      }
  ...
  static async detail(req, res) {
    try {
      ...
      if (!employee) {
        return res.status(404).json({ status: 'error', message: messages.EMPLOYEE.NOT_FOUND });
      }
  ...
  static async report(req, res) {
    try {
      const employees = await Employee.findAll({
        include: [
          { model: EmployeeProfile, as: 'profile', attributes: ['place_of_birth', 'date_of_birth', 'gender'] },
          { model: Education, as: 'education', attributes: ['name', 'level'] },
          { model: EmployeeFamily, as: 'families', attributes: ['relation_status'] }
        ]
      });
  ...
  static async create(req, res) {
    ...
    try {
      ...
      await t.rollback();
      console.error(error);
      return res.status(500).json({ status: 'error', message: messages.EMPLOYEE.CREATE_FAILED });
    }
  }
  ...
  static async update(req, res) {
    ...
    try {
      ...
      await t.rollback();
      console.error(error);
      return res.status(500).json({ status: 'error', message: messages.EMPLOYEE.UPDATE_FAILED });
    }
  }
  ...
  static async delete(req, res) {
    ...
    try {
      ...
      await Education.destroy({ where: { employee_id: employee.id }, transaction: t });
      await EmployeeFamily.destroy({ where: { employee_id: employee.id }, transaction: t });
      await EmployeeProfile.destroy({ where: { employee_id: employee.id }, transaction: t });
      ...
      await t.rollback();
      console.error(error);
      return res.status(500).json({ status: 'error', message: messages.EMPLOYEE.DELETE_FAILED });
    }
  }
